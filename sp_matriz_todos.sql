...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_obtener_registros_matriz_todos`(
...
BEGIN
    ...
    WHERE FECHA >= DATE_SUB(v_fecha_ref, INTERVAL 12 MONTH)
      -- MODIFICACIÓN: Si p_codigo_pre es NULL o vacío, no filtrar por IPRESS
      AND (p_codigo_pre IS NULL OR p_codigo_pre = '' OR LEFT(CODIGO_PRE, 5) = p_codigo_pre)
      -- MODIFICACIÓN: Si p_codigo_sismed es NULL o vacío, no filtrar por medicamento
      AND (p_codigo_sismed IS NULL OR p_codigo_sismed = '' OR CODIGO_MED = p_codigo_sismed);
    ...
    DROP TEMPORARY TABLE IF EXISTS temp_salidas_mes_siguiente;
